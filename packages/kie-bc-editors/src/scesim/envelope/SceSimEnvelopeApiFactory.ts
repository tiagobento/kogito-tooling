...
export class SceSimEditorEnvelopeApiImpl
  ...
  constructor(private readonly sceSimArgs: SceSimEnvelopeApiFactoryArgs) {
    super(sceSimArgs, new SceSimEditorFactory());
  }

  public mySceSimEnvelopeMethod() {
    this.sceSimArgs.envelopeContext.channelApi.notifications.mySceSimChannelMethod();
    const editor = this.sceSimArgs.view().getEditor();
    const ret = editor?.mySceSimMethod() ?? "scesim-return--default";
