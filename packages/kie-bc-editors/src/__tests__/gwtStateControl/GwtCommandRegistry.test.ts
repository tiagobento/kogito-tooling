...
describe("DefaultKogitoCommandRegistry", () => {
  ...
  test("test basic add/remove elements", () => {
    ...
    expect(channelApi.notifications.receive_newEdit.send).toBeCalledWith(expect.objectContaining({ id: "1" }));

    registry.register(COMMAND2.getId, COMMAND2);
    expect(channelApi.notifications.receive_newEdit.send).toBeCalledWith(expect.objectContaining({ id: "2" }));

    registry.register(COMMAND3.getId, COMMAND3);
    expect(channelApi.notifications.receive_newEdit.send).toBeCalledWith(expect.objectContaining({ id: "3" }));

    registry.register(COMMAND4.getId, COMMAND4);
    expect(channelApi.notifications.receive_newEdit.send).toBeCalledWith(expect.objectContaining({ id: "4" }));

    expect(channelApi.notifications.receive_newEdit.send).toBeCalledTimes(4);
  ...
  test("test adding reaching max elements", () => {
    ...
    expect(channelApi.notifications.receive_newEdit.send).toBeCalledTimes(2);
  ...
  test("test basic adding elements after remove", () => {
    ...
    registry.register(COMMAND3.getId, COMMAND3);
    expect(channelApi.notifications.receive_newEdit.send).toBeCalledWith(expect.objectContaining({ id: "3" }));
    expect(channelApi.notifications.receive_newEdit.send).toBeCalledTimes(3);
    expect(registry.getCommands()).toHaveLength(3);
    expect(registry.getCommands()).toContain(COMMAND1);
    expect(registry.getCommands()).toContain(COMMAND2);
    expect(registry.getCommands()).toContain(COMMAND3);
