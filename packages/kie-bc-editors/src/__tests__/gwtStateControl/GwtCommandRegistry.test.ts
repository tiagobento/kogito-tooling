...
import { GwtCommandRegistryImpl } from "../../gwtStateControl";
...
let kogitoChannelApiClient: MessageBusClient<KogitoChannelApi>;
let registry: GwtCommandRegistryImpl<Command>;
...
describe("DefaultKogitoCommandRegistry", () => {
  beforeEach(() => {
    ...
    registry = new GwtCommandRegistryImpl<Command>(kogitoChannelApiClient);
  });

  test("test basic add/remove elements", () => {
    registry.register(COMMAND1.getId, COMMAND1);
    expect(kogitoChannelApiClient.notify).toBeCalledWith("receive_newEdit", expect.objectContaining({ id: "1" }));

    registry.register(COMMAND2.getId, COMMAND2);
    expect(kogitoChannelApiClient.notify).toBeCalledWith("receive_newEdit", expect.objectContaining({ id: "2" }));

    registry.register(COMMAND3.getId, COMMAND3);
    expect(kogitoChannelApiClient.notify).toBeCalledWith("receive_newEdit", expect.objectContaining({ id: "3" }));

    registry.register(COMMAND4.getId, COMMAND4);
    expect(kogitoChannelApiClient.notify).toBeCalledWith("receive_newEdit", expect.objectContaining({ id: "4" }));

    expect(kogitoChannelApiClient.notify).toBeCalledTimes(4);
  ...
  test("test adding reaching max elements", () => {
    ...
    expect(kogitoChannelApiClient.notify).toBeCalledTimes(2);
  ...
  test("test basic adding elements after remove", () => {
    ...
    registry.register(COMMAND3.getId, COMMAND3);
    expect(kogitoChannelApiClient.notify).toBeCalledWith("receive_newEdit", expect.objectContaining({ id: "3" }));
    expect(kogitoChannelApiClient.notify).toBeCalledTimes(3);
    expect(registry.getCommands()).toHaveLength(3);
    expect(registry.getCommands()).toContain(COMMAND1);
    expect(registry.getCommands()).toContain(COMMAND2);
    expect(registry.getCommands()).toContain(COMMAND3);
