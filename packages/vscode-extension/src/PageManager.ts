...
export class PageManager {
    constructor(
        ...
        private readonly pageEnvelopeLocator: PageEnvelopeLocator,
  ...
  get pages() {
    return [...this.pageEnvelopeLocator.mapping.entries()].map(([pageId]) => ({
      id: pageId,
  ...
  public open(pageId: string) {
    const pageMapping = this.pageEnvelopeLocator.mapping.get(pageId);
    if (!pageMapping) {
    ...
    const api: KogitoPageChannelApi = new KogitoPageChannelApiImpl(this.workspaceApi, this.editorStore);
    ...
    envelopeServer.startInitPolling(this.pageEnvelopeLocator.targetOrigin, {
      filePath: filePath,
